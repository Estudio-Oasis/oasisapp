            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[11px] bg-background-secondary border border-border rounded-full px-2 py-0.5">{colTasks.length}</span>
                </div>
                <button
                  ...
                  {colTasks.map((task) => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-card border border-border/60 dark:border-border/40 rounded-2xl p-4 cursor-pointer hover:border-border transition-all"
                      >
                        ...
                        <p className="text-[13px] font-semibold text-foreground line-clamp-2">{task.title}</p>
                        <div className="flex items-center justify-between mt-2">
                          {task.due_date ? (
                            <span className={`text-[11px] ${overdue ? "text-destructive font-semibold" : "text-foreground-muted"}`}>
                              {new Date(task.due_date).toLocaleDateString("es-MX", { month: "short", day: "numeric" })}
                            </span>
                          ) : <span />}
                          {assignee && (
                            <div
                              className="h-5 w-5 rounded-full flex items-center justify-center text-[9px] font-semibold text-background"
                              style={{ backgroundColor: getClientColor(assignee.name || "U") }}
                            >
                              {(assignee.name || "U").slice(0, 2).toUpperCase()}
                            </div>
