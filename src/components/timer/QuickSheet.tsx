...
export function QuickSheet({ open, onOpenChange, mode = "start" }: QuickSheetProps) {
  ...
  }, [mode, startTimer, switchTask, onOpenChange, notes]);
            ...
            <div className="space-y-0.5">
              {recents.map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => handleStart(item.description, item.clientId, item.taskId, item.projectId)}
                    disabled={loading}
