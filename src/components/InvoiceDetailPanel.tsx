        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="border border-border rounded-lg p-4">
              <p className="text-micro text-foreground-muted mb-1">Amount</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-micro text-foreground-muted mb-1">Due date</p>
                <p className="text-sm text-foreground">
                  {invoice.due_date ? new Date(invoice.due_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}
                </p>
              </div>
              <div>
                <p className="text-micro text-foreground-muted mb-1">Paid at</p>
                <p className="text-sm text-foreground">
            ...
            {(invoice.period_start || invoice.period_end) && (
              <div>
                <p className="text-micro text-foreground-muted mb-1">Period</p>
                <p className="text-sm text-foreground">
                  {invoice.period_start ? new Date(invoice.period_start + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}
                  {" → "}
                  {invoice.period_end ? new Date(invoice.period_end + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}
                </p>
              </div>
            )}
            ...
            <p className="text-micro text-foreground-muted">Actions</p>
