...
serve(async (req) => {
  ...
  try {
    ...
    if (hasActive) {
      ...
      await supabaseClient.from("profiles").update({ plan: "pro" }).eq("id", user.id);
    } else {
    ...
    return new Response(
      JSON.stringify({ subscribed: hasActive, product_id: productId, subscription_end: subscriptionEnd }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
